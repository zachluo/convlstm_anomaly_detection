...
template <typename Dtype>
void ConvolutionLSTMUnitLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  ConvLSTMActsForward<Dtype><<<CAFFE_GET_BLOCKS(X_count), CAFFE_CUDA_NUM_THREADS>>>(
      X_count, hidden_dim_ * height * width, X, X_acts);
  CUDA_POST_KERNEL_CHECK;
  // NOLINT_NEXT_LINE(whitespace/operators)
  ConvLSTMUnitForward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
      count, hidden_dim_ * height * width, C_prev, X_acts, cont, C, H);
  CUDA_POST_KERNEL_CHECK;
}
...
template <typename Dtype>
void ConvolutionLSTMUnitLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  ConvLSTMUnitBackward<Dtype>  // NOLINT_NEXT_LINE(whitespace/operators)
      <<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(count, hidden_dim_ * height * width,
  ...
  ConvLSTMActsBackward<Dtype>  // NOLINT_NEXT_LINE(whitespace/operators)
      ...
      X_count, hidden_dim_ * height * width, X_acts, X_acts_diff, X_diff);
  CUDA_POST_KERNEL_CHECK;
}

INSTANTIATE_LAYER_GPU_FUNCS(ConvolutionLSTMUnitLayer);
