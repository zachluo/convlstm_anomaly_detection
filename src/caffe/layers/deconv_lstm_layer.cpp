...
void DeconvolutionLSTMLayer<Dtype>::RecurrentInputBlobNames(vector<string>* names) const {
  names->resize(2);
  (*names)[0] = "h_0";
  (*names)[1] = "c_0";
}
...
template <typename Dtype>
void DeconvolutionLSTMLayer<Dtype>::RecurrentOutputBlobNames(vector<string>* names) const {
  names->resize(2);
 ...
 */
template <typename Dtype>
void DeconvolutionLSTMLayer<Dtype>::RecurrentInputShapes(vector<BlobShape>* shapes) const {
...
//			this->layer_param_.recurrent_param().whh_convolution_param();
	...
	int H_out_ = (this->H_ - 1) * wxc_conv_param.stride(0) + wxc_conv_param.kernel_size(0) - 2 * wxc_conv_param.pad(0);
	int W_out_ = (this->W_ - 1) * wxc_conv_param.stride(0) + wxc_conv_param.kernel_size(0) - 2 * wxc_conv_param.pad(0);
  ...
  for (int i = 0; i < num_blobs; ++i) {
    ...
    // updates 2016-10-04
    (*shapes)[i].add_dim(H_out_);  	// H
    (*shapes)[i].add_dim(W_out_);		// W
  }
}

template <typename Dtype>
void DeconvolutionLSTMLayer<Dtype>::OutputBlobNames(vector<string>* names) const {
  names->resize(1);
  (*names)[0] = "h";
}

template <typename Dtype>
void DeconvolutionLSTMLayer<Dtype>::FillUnrolledNet(NetParameter* net_param) const {
 ...
 /* CHECK_EQ(this->H_, (this->H_ - wxc_conv_param.kernel_size(0) + 2 * wxc_conv_param.pad(0)) /
  ...
  wxc_biased_conv_param.set_type("Deconvolution");
  ...
  whh_unbiased_conv_param.set_type("Deconvolution");
...
INSTANTIATE_CLASS(DeconvolutionLSTMLayer);
REGISTER_LAYER_CLASS(DeconvolutionLSTM);
