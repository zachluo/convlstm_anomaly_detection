...
#ifndef CONV_LSTM_LAYER_HPP_
#define CONV_LSTM_LAYER_HPP_
...
class ConvolutionLSTMLayer : public RecurrentLayer<Dtype> {
 public:
  explicit ConvolutionLSTMLayer(const LayerParameter& param)
...
template <typename Dtype>
class DeconvolutionLSTMLayer : public RecurrentLayer<Dtype> {
 public:
  explicit DeconvolutionLSTMLayer(const LayerParameter& param)
      : RecurrentLayer<Dtype>(param) {}

  virtual inline const char* type() const { return "DeonvolutionLSTM"; }
 ...
 */
template <typename Dtype>
class ConvolutionLSTMUnitLayer : public Layer<Dtype> {
 public:
  explicit ConvolutionLSTMUnitLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline const char* type() const { return "ConvolutionLSTMUnit"; }
 ...
 protected:
   ...
   *   -# @f$ (1 \times N \times D \times W \times H) @f$
   *      the previous timestep cell state @f$ c_{t-1} @f$
   *   -# @f$ (1 \times N \times 4D \times W \times H) @f$
